...
function filterUnwantedWords(words) {
  // Eliminar palabras que repitan letras, o que tengan las letras y, q, h, f, z, j, ñ, x, k, w (ya no sa necesita)
  return words.filter(
    (word) => new Set(word).size === word.length // && !/[yqhfzjñxkw]/.test(word)
  );
}
...
function filtrarPalabrasConUnaSolaVocal(words) {
  return words.filter((word) => {
    ...
    const uniqueVowels = new Set(
      word.split("").filter((letter) => vowels.includes(letter))
    );
    return uniqueVowels.size === 1;
  });
}

function copyWordsWithFiveLetters() {
  const wordsWithFiveLetters = filteredWords.filter(
    (word) => word.length === 5
  );
  // copiar al portapapeles como texto plano con 1 palabra por línea (5 letras)
  navigator.clipboard.writeText(wordsWithFiveLetters.join("\n"));
}

function copyWordsWithFiveLettersAsArray() {
  const wordsWithFiveLetters = filteredWords.filter(
    (word) => word.length === 5
  );
  // copiar como array
  navigator.clipboard.writeText(JSON.stringify(wordsWithFiveLetters));
}
